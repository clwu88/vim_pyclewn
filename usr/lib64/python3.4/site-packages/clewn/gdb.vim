...
augroup clewn
    ...
    autocmd BufEnter (clewn)_backtrace nnoremap <buffer> <silent> t :call <SID>goto_file_tab_stack()<CR>
...
function! s:parse_filename_linenum(linenum)
    let l:line = getline(a:linenum)
    let l:regexp = '^.\+:\(\d\+\) <\(.\+\)>$'
    let l:file_linenum = substitute(l:line, l:regexp , '\1', "")

    if l:file_linenum == l:line
        let l:file_linenum = -1
        let l:filename     = ''
    else
        let l:filename = substitute(l:line, l:regexp , '\2', "")
        if l:filename == l:line
            let l:file_linenum = -1
            let l:filename     = ''
        endif
    endif

    return [ l:file_linenum, l:filename ]
endfunction

function! s:parse_filename_linenum_list(_firstline, _lastline)
    let l:list = []
    let l:i = a:_firstline

    while l:i <= a:_lastline
        call insert( l:list, s:parse_filename_linenum(l:i), 0 )
        let l:i += 1
    endwhile
...
function! <SID>goto_file_tab_stack() range
    let l:list = s:parse_filename_linenum_list(a:firstline, a:lastline)
    let l:is_first = 1

    for [ l:file_linenum, l:filename ] in l:list
        if -1 != l:file_linenum
            if 1 == l:is_first
                exe 'tabnew +'.l:file_linenum.' '.l:filename
                let l:is_first = 0
            else
                exe 'split +'.l:file_linenum.' '.l:filename
            endif
        endif
    endfor
endfunction
